...
public class RNDeviceModule extends ReactContextBaseJavaModule {
  ...
  public void getMacAddress(Promise p) {
    ...
    ShowAlert("Wifi Mac Address: " + macAddress);
    ...
    if (res == PackageManager.PERMISSION_GRANTED) {
      try {
        ...
        if(all == null || all.size() == 0){
          ShowAlert("Không lấy được danh sách mạng kết nối");
        }

        String error = "";
        for (NetworkInterface nif : all) {
          if (!nif.getName().equalsIgnoreCase("wlan0")){
            error = "Tên mạng là wlan0";
            continue;
          }
          ...
          if (macBytes == null) {
            macAddress = "";
            error = "Không lấy được địa chỉ của phần cứng";
          } else {
        ...
        if(!macAddress.isEmpty()){
          ShowAlert("Địa chỉ MacAddress Sim: " + macAddress);
        }else{
          ShowAlert("Không lấy được địa chỉ trên phần cứng: " + error);
        }
      } catch (Exception ex) {
        ShowAlert("Lỗi xảy ra: " + ex.getMessage());
      }
    }else{
      ShowAlert("Chưa có quyền lấy địa chỉ Mac");
    }
    p.resolve(macAddress);
  }
  ...
  private void ShowAlert(String alert) {
    try {
      ...
      new AlertDialog.Builder(
              getCurrentActivity(), R.style.Theme_AppCompat_Dialog)
              .setTitle("kiểm tra Mac Address")
              .setMessage(alert)
              .setCancelable(false)
